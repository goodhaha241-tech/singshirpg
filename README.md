...
## ⚙️ 데이터베이스 GUI 도구 연결 (VS Code, DBeaver 등)

팀원과 협업 시, 코드를 직접 실행하지 않고 데이터베이스의 내용을 확인하거나 수정하고 싶을 때 GUI 도구를 사용할 수 있습니다.

1.  **연결 정보 확인**:
    *   **Host**: 호스트(DB 공유자)의 Hamachi IP 주소 (예: `25.x.x.x`)
    *   **Port**: `3306`
    *   **User**: `friend` (또는 호스트가 생성해준 사용자 이름)
    *   **Password**: 호스트가 설정한 비밀번호
    *   **Database**: `discord_bot_db`

2.  **연결 설정**:
    *   사용하는 MySQL 확장 프로그램이나 GUI 도구(DBeaver, MySQL Workbench 등)에서 '새 연결(New Connection)'을 생성합니다.
    *   위에서 확인한 정보를 각 필드에 정확히 입력하고 연결을 테스트합니다.

3.  **주의사항**:
    *   이 방법으로 연결하려면 **호스트와 클라이언트 모두 Hamachi(또는 Radmin VPN)에 연결되어 있어야 합니다.**
    *   호스트의 컴퓨터나 MySQL 서버가 꺼져 있으면 접속할 수 없습니다.