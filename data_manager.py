...
async def _get_new_user_data(user_name=None):
    ...
    return {
        ...
        "myhome": {
            ...
            "max_subjugation_char": "",
            "max_subjugation_region": ""
        },
        "daily_quests": [],
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            try:
                ...
                m_char = myhome.get("max_subjugation_char") or ""
                m_region = myhome.get("max_subjugation_region") or ""
...
async def get_subjugation_ranking(limit=10, region=None):
    """최대 탐사 층수 기준 상위 유저를 가져옵니다. 지역 필터링이 가능합니다."""
    pool = await get_db_pool()
    async with pool.acquire() as conn:
        async with conn.cursor(aiomysql.DictCursor) as cur:
            if region:
                sql = """
                    SELECT user_id, max_subjugation_depth, max_subjugation_char, max_subjugation_region 
                    FROM users 
                    WHERE max_subjugation_depth > 0 AND max_subjugation_region = %s
                    ORDER BY max_subjugation_depth DESC 
                    LIMIT %s
                """
                await cur.execute(sql, (region, limit))
            else:
                sql = """
                    SELECT user_id, max_subjugation_depth, max_subjugation_char, max_subjugation_region 
                    FROM users 
