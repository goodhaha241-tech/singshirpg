...
class ItemUseView(discord.ui.View):
    ...
    async def on_item_select(self, interaction: discord.Interaction):
        ...
        # [신규] 던전 탐사 중인 캐릭터 제한
        curr_dungeon = self.user_data.get("current_dungeon", {})
        if curr_dungeon and curr_dungeon.get("char_index") == self.char_index:
            return await interaction.followup.send("❌ 던전 탐사 중인 캐릭터는 아이템을 사용할 수 없습니다. 던전에서 완전히 나오거나 탐사를 마쳐주세요.", ephemeral=True)
