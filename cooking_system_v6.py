...
def ensure_cooking_data(user_data: dict[str, Any]) -> dict[str, Any]:
    ...
    unlocked[:] = [
        ...
        if recipe_name not in RETIRED_CRAFT_RECIPES
    ]
    foods = cooking.setdefault("foods", {})
    inventory = user_data.setdefault("inventory", {})
    for food_key, count in list(foods.items()):
        recipe_name = food_key.split(" ", 1)[1] if " " in food_key else food_key
        if recipe_name not in RETIRED_CRAFT_RECIPES:
            continue
        count = max(0, int(count or 0))
        if count:
            inventory[recipe_name] = int(inventory.get(recipe_name, 0)) + count
        foods.pop(food_key, None)
